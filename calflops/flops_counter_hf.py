...
def calculate_flops_hf(
    ...
    if empty_model is None:
        empty_model = create_empty_model(model_name=model_name, library_name=None, trust_remote_code=trust_remote_code, access_token=access_token)

    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=trust_remote_code, access_token=access_token)
    ...
    calculate_flops_pipline = CalFlopsPipline(model=empty_model, include_backPropagation=include_backPropagation, compute_bp_factor=compute_bp_factor)
    ...
    if input_shape is not None:
        ...
        kwargs = generate_transformer_input(input_shape=input_shape, model_tokenizer=tokenizer, device=device)
    else:
        kwargs = generate_transformer_input(input_shape=None, model_tokenizer=tokenizer, device=device)
    ...
    try:
        ...
        print_return = calculate_flops_pipline.print_return_model_pipline(
            units=output_unit, precision=output_precision, print_detailed=print_detailed, print_results=print_results
        )
